...
export const PROMPT_TEMPLATES = {
  ...
  GENERATE_MULTI_PANEL_COMIC: (
    ...
    return `
      ...
      - 絶対にセリフ、テキスト、吹き出しは一切含めないでください
      - 後から吹き出しとセリフをGUIで別途追加するため、会話ができるスペースを各コマに確保してください
      - 吹き出し内のセリフは生成しないでください、吹き出し自体も描かないでください
      - 各コマの人物は会話しているように、表情豊かに描いてください
      - 文字、数字、記号など、テキスト要素は一切画像に含めないでください
      - すべてのテキストは後からGUIで追加するため、画像内に一切含めないでください
      ...
      最終確認: この画像には文字、吹き出し、セリフを一切含めないでください。それらは後から別途追加します。
